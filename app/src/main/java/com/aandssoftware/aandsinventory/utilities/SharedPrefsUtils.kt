...
class SharedPrefsUtils {
    ...
    companion object {
        ...
        @JvmStatic
        fun setUserPreference(context: Context, key: String, customerModel: CustomerModel?): Boolean {
            val preferences = context.getSharedPreferences(context.getString(R.string.app_name), Context.MODE_PRIVATE)
            if (preferences != null) {
                val editor = preferences.edit()
                if (customerModel != null) {
                    editor.putString(key, Gson().toJson(customerModel))
                } else {
        ...
        @JvmStatic
        fun getUserPreference(context: Context, key: String): CustomerModel? {
            var value: CustomerModel? = null
            val preferences = context.getSharedPreferences(context.getString(R.string.app_name), Context.MODE_PRIVATE)
            if (preferences != null) {
                val data = preferences.getString(key, null)
                data?.let {
                    value = Gson().fromJson(data, CustomerModel::class.java)
                }
            }
            return value
        }
